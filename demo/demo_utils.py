...
def plot_similarity_matrix(matrix,
    ...
    img = ax.matshow(matrix,
                     extent=(-0.5, matrix.shape[0] - 0.5, -0.5,
                             matrix.shape[1] - 0.5))
...
def plot_projections(embeds,
    ...
    if legend:
        ax.legend(title="Speakers", ncol=2, loc='best')
    ax.set_title(title)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_aspect("equal")
...
def interactive_diarization(similarity_dict,
                            wav,
                            wav_splits,
                            x_crop=5,
                            show_time=False):
    ...
    def update(i):
        ...
        name, similarity = list(
            similarity_dict.keys())[best], similarities[best]
        if similarity > 0.75:
